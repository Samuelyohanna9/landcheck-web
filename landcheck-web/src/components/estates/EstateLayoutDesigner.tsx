...
// The templates below are all defined in metres - when an estate works in feet, appending the
// converted size/frontage lets a user pick a template without doing the m->ft math themselves.
const describeTemplateSize = (template: LayoutTemplate, unit: UnitSystem): string => {
  if (unit !== "ft") return template.plotSizeLabel;
  const areaSqm = template.criteria.target_plot_area_sqm;
  const frontageM = template.criteria.frontage_m;
  if (!areaSqm) return template.plotSizeLabel;
  const areaFt = Math.round(sqmToSqft(areaSqm));
  const depthM = frontageM ? areaSqm / frontageM : null;
  const dims = frontageM && depthM ? ` (≈${Math.round(metersToFeet(frontageM))} x ${Math.round(metersToFeet(depthM))}ft)` : "";
  return `${template.plotSizeLabel} / ${areaFt.toLocaleString()} ft²${dims}`;
};
  ...
  useEffect(() => {
    ...
    void Promise.all([loadMapboxGl(), loadMapboxGlCss()]).then(([mapboxgl]) => {
      ...
      map.on("style.load", () => {
        ...
        // Hundreds of small plots at once means a label wider than its own plot bleeds straight
        // into its neighbours - a much higher minzoom (so labels only draw once each plot is
          ...
          <>
            ...
            <label className="edash-field" style={{ marginBottom: 10, maxWidth: 360 }}>
              ...
              <select value={templateKey} onChange={(event) => applyTemplate(event.target.value as LayoutTemplateKey)}>
                {LAYOUT_TEMPLATES.map((template) => <option key={template.key} value={template.key}>{template.name} - {describeTemplateSize(template, unitSystem)}</option>)}
              </select>
            </label>
